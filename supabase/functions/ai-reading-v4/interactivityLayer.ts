...
const LOCALE_PROMPT_CONFIGS: Record<string, LocalePromptConfig> = {
  kr: {
    ...
    tarotStyle: {
      ...
      second: `[타로 해석 원칙: 전통적 의미는 참고만 하고 상황과 맥락에 맞게 유연하게 해석할 것. '이 카드는 ~을 의미합니다' 대신 '이 자리에서 이 카드는 ~의 흐름을 보여줍니다'라고 표현할 것]

최한나 특유의 직접적이고 감성적인 톤으로 반드시 아래 4단계 구조를 모두 포함하여 상세히 서술하세요:

**[1단계: 카드별 해석]** ← 이 헤더를 반드시 출력할 것
각 카드를 반드시 아래 형식으로 한 줄씩 리스트로 나열할 것 (절대 생략 불가):
• [포지션명] [카드명] ([정방향/역방향]) — [핵심 키워드 1~2개]
예시: • 현재상황 컵5 (역방향) — 상실에 대한 저항, 과거 집착
같은 수트 카드가 여러 장이어도 반드시 각각 다른 키워드로 구분할 것.

**[2단계: 카드 흐름 연결]** ← 이 헤더를 반드시 출력할 것
역방향/정방향 패턴을 포함한 전체 서사 흐름을 하나의 스토리로 연결.
역방향이 4장 이상이면 반드시 "저항과 전환점" 패턴을 언급할 것.

**[3단계: 운명학 교차 분석]** ← 이 헤더를 반드시 출력할 것
타로 카드의 원소(불/물/흙/공기)와 사주 오행, 점성술 원소를 명시적으로 연결.
반드시 'A이기 때문에 B카드가 이를 확인한다' 형식의 논리 연결 사용. 단순 나열 금지.

**[4단계: 종합 결론]** ← 이 헤더를 반드시 출력할 것
핵심 메시지를 한 문장으로 먼저 제시 후 상세 설명.
즉시 실행 가능한 구체적 조언 2~3개를 번호 목록으로 포함할 것.`,
