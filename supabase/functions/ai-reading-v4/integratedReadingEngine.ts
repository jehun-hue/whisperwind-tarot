...
import { lunarToSolar as lunarToSolarCore } from "./lib/lunarConverter.ts";
import { 
  CITY_COORDINATES, LUCKY_MAP, TOPIC_MAPPING, 
  CATEGORY_KOREAN, TOPIC_PATTERNS, SOLAR_TERMS_KR, 
  SYMBOLIC_MEANINGS,
  transformAstrologyData, transformZiweiData,
  generateFallbackAstrology, generateFallbackZiwei,
  getPillarFromData, getDayMasterFromData, getYongShinFromData,
  convertLunarToSolar
} from "./lib/dataTransform.ts";
 ...
 * - 자미두수: 음력(Lunar) 기준 → rawBirth 또는 convertLunarToSolar 변환값 사용
 * - 수비학: 양력 기준
 * - 타로: 날짜 무관 (카드 기반)
 * 
...
export function buildEnginePrompts(input: any, sajuRaw: any, sajuAnalysis: any, ziweiAnalysis?: any, astrologyAnalysis?: any, ageContext?: any) {
  ...
  const sajuDisplay = {
    ...
    termName: (sajuRaw?.termIdx !== undefined) ? SOLAR_TERMS_KR[sajuRaw.termIdx] : "알 수 없음",
...
export async function runFullProductionEngineV8(supabaseClient: any, apiKey: string, input: any) {
  ...
  if (rawBirth?.year !== undefined) {
    ...
    birthInfo = {
      ...
      ...(() => {
        ...
        const exactMatch = CITY_COORDINATES[place];
        if (exactMatch) return { birthPlace: bPlace, latitude: exactMatch[0], longitude: exactMatch[1] };
        const partialKey = Object.keys(CITY_COORDINATES).find(k => place.includes(k) || k.includes(place));
        if (partialKey) return { birthPlace: bPlace, latitude: CITY_COORDINATES[partialKey][0], longitude: CITY_COORDINATES[partialKey][1] };
  ...
  // Step 1: Physical Calculation Pipeline
    ...
    if (isLunar) {
      ...
      const lunarResult = convertLunarToSolar(sY, sM, sD);
...
- 충돌: ${consensusResult?.conflict_summary || '없음'}
      ...
      parsed.tarot_reading = {
        choihanna: { ...cardData, story: finalChoihanna, key_message: "통합 분석에 기반한 위스퍼윈드의 제언입니다." },
        monad: { ...cardData, story: finalMonad, key_message: "통합 분석에 기반한 위스퍼윈드의 제언입니다." },
        e7l3: { ...cardData, story: finalChoihanna, key_message: "통합 분석에 기반한 위스퍼윈드의 제언입니다." },
        e5l5: { ...cardData, story: thirdNarrative, key_message: "통합 분석에 기반한 위스퍼윈드의 제언입니다." },
        l7e3: { ...cardData, story: finalMonad, key_message: "통합 분석에 기반한 위스퍼윈드의 제언입니다." }
      };
    } catch (e: any) {
  ...
  logMonitoringEvent(supabaseClient, {
    ...
    promptTokensEstimate: Math.round(modelInputSummary.length / 4),
    questionType,
    ...
    rawResponsePreview: JSON.stringify(parsed?.tarot_reading)?.slice(0, 500),
  });
...
`.trim();
  ...
  return {
    ...
    error_message: null,
    raw_narrative: JSON.stringify(parsed?.tarot_reading),
    debug_prompt: stylePrompts?.join("\n\n") || "",
